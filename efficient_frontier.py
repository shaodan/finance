...
def acquire_data(data_path='data/stock_prices.csv',index_col="Date", n_companies=0):
    """
...
def efficient_frontier_optimisation(adj_close, mean_returns, cov_matrix, risk_free_rate=0, monte_carlo=None, ax=None):
    ...
    target = np.linspace(mean_returns.min(),
                         mean_returns.max(), 50)
...
if __name__ == "__main__":
    df = acquire_data('reits_clean.csv', index_col='Dates')
    # df_without_reits = df.drop(columns=['FNERTR Index'])
    # df_without_reits = df.drop(columns=['LF98TRUU Index'])
    df_without_reits = df.drop(columns=['XAU Curncy'])

    fig, ax = plt.subplots(figsize=(9, 6))

    daily_stock_returns, mean_stock_returns, cov_annual = preprocess_data(df)
    efficient_frontier_optimisation(df, mean_stock_returns, cov_annual, ax=ax)
    # efficient_frontier_optimisation(df, mean_stock_returns, cov_annual, monte_carlo=10000)

    daily_stock_returns, mean_stock_returns, cov_annual = preprocess_data(df_without_reits)
    efficient_frontier_optimisation(df_without_reits, mean_stock_returns, cov_annual, ax=ax)
    # efficient_frontier_optimisation(df_without_reits, mean_stock_returns, cov_annual, monte_carlo=10000)
